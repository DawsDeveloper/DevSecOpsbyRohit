...
# 🌐 Example 3: Create a LAMP Architecture Using Docker

LAMP stands for:

- **L** – Linux (all Docker containers run on Linux base)
- **A** – Apache (Web server)
- **M** – MySQL (Database)
...
## ✅ Option 1: Using Docker User-Defined Bridge Network

This approach manually connects containers over a shared Docker network.
...
docker run -d \
  --name mysql \
  --network lamp-net \
  -e MYSQL_ROOT_PASSWORD=rohit \
  mysql:5.7
```
### 🔧 Step 3: Run Apache + PHP Container
...
docker run -d \
  ...
  php:7.4-apache
```
📂 PHP files go inside: /var/www/html
...
```
### Use vi /var/www/html/index.php to add PHP code. PHP can connect to MySQL using:
...
## ✅ Option 2: Docker Compose with Service-Based Networking

This is the modern and scalable approach for multi-container LAMP architecture.
...
└── php/
    └── index.php
lamp/ → Root project folder

docker-compose.yml → Defines MySQL and PHP-Apache services

php/index.php → Sample PHP file that will be served on the web server
```
### ✍️ Example `php/index.php`

Create a file named `index.php` inside the `php/` directory with the following content:

```php
<?php
$mysqli = new mysqli("mysql", "root", "rohit");
...
```
▶️ Start the Setup
Run this command inside the lamp/ directory:
```bash
docker-compose up -d
